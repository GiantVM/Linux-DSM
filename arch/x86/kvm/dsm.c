...
static int kvm_dsm_vcpu_page_fault(struct kvm_vcpu *vcpu, struct kvm_memory_slot *memslot,
		gfn_t gfn, bool is_smm, int write);
static int kvm_dsm_vcpu_page_fault_async(struct kvm_vcpu *vcpu, struct kvm_memory_slot *memslot,
		gfn_t gfn, bool is_smm, int write);
...
static int __kvm_dsm_vcpu_acquire_page_async(struct kvm_vcpu *vcpu,
		struct kvm_memory_slot *slot, gfn_t gfn, bool is_smm, bool write)
{
	struct kvm_dsm_memory_slot *hvaslot;
	hfn_t vfn;
		...
		struct kvm_memory_slot **slot, gfn_t gfn, bool write)
{
	...
	if (slot)
		*slot = memslot;
	return __kvm_dsm_vcpu_acquire_page_async(vcpu, memslot,
			gfn, is_smm(vcpu), write);
}
...
#ifdef IVY_KVM_DSM
	ret = ivy_kvm_dsm_vcpu_page_fault_async(vcpu, memslot, gfn, is_smm, write);
#elif defined(TARDIS_KVM_DSM)
	ret = tardis_kvm_dsm_page_fault(kvm, memslot, gfn, is_smm, write);
#endif
